...
struct queue<T>::node_t {
  using slot_array_t = std::array<queue::atomic_slot_t, NODE_SIZE>;
  ...
  explicit node_t(pointer first) : node_t() {
    this->slots[0].store(reinterpret_cast<queue::slot_t>(first), relaxed);
  }
  ...
  void increment_dequeue_count(std::uint64_t final_count = 0) {
    constexpr auto EXPECTED_FLAGS = reclaim_flags_t::ENQ | reclaim_flags_t::ARR;
    assert(final_count < std::numeric_limits<std::uint16_t>::max());
    const auto counts = final_count == 0
...
private:
  ...
  static counts_t increment_counter(std::atomic_uint32_t& counter) {
    const auto mask = counter.fetch_add(1, release);
  ...
  static counts_t increment_counter_final(
    ...
    const auto add = 1 + (std::uint32_t{ final_count } << counter_flags_t::SHIFT);
